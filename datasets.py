...
def calc_damp_gt(size, points):
    k = np.zeros((size, size))
    for i in range(0, len(points)):
        if(int(points[i][1]) < size and int(points[i][0]) < size):
...
class Crowd(data.Dataset):
    ...
    def train_transform(self, img, keypoints):
        ...
        if len(keypoints) > 0:
            ...
            if random.random() > 0.5:
                img = F.hflip(img)
        if not self.two_branch:
            return self.trans(img), torch.from_numpy(keypoints.copy()).float(), \
                torch.from_numpy(target.copy()).float(), st_size
        else:
            damp_gt = calc_damp_gt(self.c_size, keypoints)
            return self.trans(img), torch.from_numpy(keypoints.copy()).float(), \
                torch.from_numpy(target.copy()).float(), st_size, \
                cv2.resize(damp_gt,(damp_gt.shape[1]//8,damp_gt.shape[0]//8),interpolation = cv2.INTER_CUBIC)*64